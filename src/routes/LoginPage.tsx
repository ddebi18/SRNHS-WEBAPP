...
import { LogIn, Camera, Lock, Mail, ShieldAlert, Eye, EyeOff, Clock } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cleanId = sanitizeInput(email, 120);
    const cleanPassword = password.trim();

    if (!cleanId) {
      setErrorMessage('Please enter your username or institutional email.');
      return;
    }

    setLoading(true);
    const result = await login(cleanId, cleanPassword, role);
    ...
    if (result.success) {
      ...
      if (rateLimitStatus.isLocked) {
        ...
        setErrorMessage(result.error || 'Invalid institutional credentials. Please verify and try again.');
      }
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-sidebar flex items-center justify-center shadow-card-sm">
          ...
          <div>
            <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 leading-tight">
              Authorized Staff<br />Sign-In Portal
            </h1>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {['🎯 Real-time Turnstile Scans', '📸 Facial Recognition Boundary', '📱 Automated Parent SMS', '🔒 DepEd Data Compliance'].map(f => (
          ...
          <div className="flex items-start gap-3 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-card-sm">
            ...
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">
              Camera capture and face-matching run on dedicated edge turnstile nodes. This dashboard securely stores attendance logs and dispatches guardian alerts.
            </p>
          </div>
        </motion.div>
        ...
        >
          <div>
            <h2 className="text-xl font-black text-slate-900 dark:text-slate-100">Sign In to Dashboard</h2>
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">
              Enter your institutional credentials to access your session.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="admin or user@srnhs.edu.ph"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 text-sm rounded-2xl font-medium
                  bg-slate-50 dark:bg-slate-800/80
                  border border-slate-200 dark:border-slate-700
                  text-slate-900 dark:text-slate-100
                  focus:outline-none focus:ring-2 focus:ring-brand-500 dark:focus:ring-brand-400 transition-all cursor-pointer disabled:opacity-50"
              >
                <option value="admin">School Administrator (Full Institutional Access)</option>
                <option value="teacher">Faculty Member (Assigned Section Scope)</option>
              </select>
            </div>
